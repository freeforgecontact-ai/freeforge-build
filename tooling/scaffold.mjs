...
if (target === 'android') {
  ...
  fs.writeFileSync(path.join(outDir, 'package.json'), JSON.stringify({ name: 'ff-' + appId, version: '1.0.0', private: true, devDependencies: { '@capacitor/cli': '^6.1.2' }, dependencies: { '@capacitor/core': '^6.1.2', '@capacitor/android': '^6.1.2' } }, null, 2));
} else {
  ...
  fs.writeFileSync(path.join(outDir, 'package.json'), JSON.stringify({ name: 'ff-' + appId, version: '1.0.0', description: appName, author: 'PGRG', main: 'main.js', scripts: { dist: 'electron-builder' }, devDependencies: { electron: '^31.7.7', 'electron-builder': '^24.13.3' }, build: { appId: bundle, productName: appName, directories: { output: 'dist' }, files: ['main.js', 'www/**/*'], win: { target: 'nsis' }, mac: { target: 'dmg', category: 'public.app-category.productivity' }, linux: { target: 'AppImage', category: 'Utility' } } }, null, 2));
}
